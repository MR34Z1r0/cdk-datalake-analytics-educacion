...
try:
    ...
    df_m_tipo_pedido_raw = spark_controller.read_table(data_paths.BIGMAGIC, "m_documento_transaccion")
...
try:
    # Aplicar transformaciones que estaban en m_tipo_pedido.py (domain)
    logger.info("Starting domain transformations - processing tipo pedido")
    df_m_tipo_pedido = (
        df_m_tipo_pedido_raw.alias("mtp")
        .join(df_m_compania.alias("mc"), col("mtp.cod_compania") == col("mc.cod_compania"), "inner")
        .join(df_m_pais.alias("mp"), col("mc.cod_pais") == col("mp.cod_pais"), "inner")
        .select(
            concat_ws("|",
                trim(col("mtp.cod_compania")),
                trim(col("mtp.cod_documento_transaccion").cast("string")),
            ).alias("id_tipo_pedido"),
            col("mp.id_pais").alias("id_pais"),
            col("mtp.cod_documento_transaccion").cast("string").alias("cod_tipo_pedido"),
            col("mtp.desc_documento_transaccion").cast("string").alias("nomb_tipo_pedido"),
            col("mtp.es_activo").cast("string").alias("estado"),
    ...
    logger.info("Starting analytics transformations - creating dim_tipo_pedido")
    df_dim_tipo_pedido = (
        df_m_tipo_pedido
        .select(
