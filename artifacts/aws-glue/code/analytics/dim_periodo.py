...
target_table_name = "dim_periodo"

try:
    # Lectura directa desde STAGE (UPEU) - equivalente a temp.Periodos
    logger.info("Reading tables from UPEU (Stage layer)")
    
    # Tabla principal Periodos (equivalente a temp.Periodos)
    df_periodos = spark_controller.read_table(data_paths.UPEU, "periodos")
...
try:
    ...
    logger.info("Starting transformations - creating dim_periodo from periodos")
    
    df_dim_periodo = (
        df_periodos
        .select(
            col("id").cast(IntegerType()).alias("id_periodo"),
            col("nombre").cast(StringType()).alias("nomb_periodo"),
    ...
    id_columns = ["id_periodo"]  # Clave para el merge
    partition_columns_array = []  # Sin partición específica, puedes agregar si necesitas
    
    logger.info(f"Starting upsert of {target_table_name}")
    spark_controller.upsert(df_dim_periodo, data_paths.ANALYTICS, target_table_name, id_columns, partition_columns_array)
    logger.info(f"Upsert de {target_table_name} success completed")
    
except Exception as e:
    logger.error(f"Error processing dim_periodo: {e}")
    raise ValueError(f"Error processing dim_periodo: {e}")